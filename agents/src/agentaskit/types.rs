...
/// Error taxonomy adapted from AgentAsKit.
///
/// The enum is intentionally rich so we can progressively map the legacy
/// AgentAsKit failure modes into the NOA ARK OS telemetry pipeline without
/// losing fidelity.
...
/// Alias for AgentAsKit results.
pub type AgentAsKitResult<T> = Result<T, AgentAsKitError>;

/// Workspace-wide configuration scaffold migrated from AgentAsKit.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Constants derived from the AgentAsKit baseline.
pub mod constants {
