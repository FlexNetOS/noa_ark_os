...
impl AgentRegistry {
    ...
    pub fn load_from_csv<P: AsRef<Path>>(&self, csv_path: P) -> Result<usize> {
        info!("Loading agent directory from: {}", csv_path.as_ref().display());
        ...
        for result in reader.records() {
            ...
            if let Ok(agent) = Self::parse_csv_record(&record) {
                ...
                if is_new {
                    ...
                    info!("Duplicate agent entry detected, keeping latest: {}", agent_id);
                }
            }
        }
    ...
    fn parse_csv_record(record: &csv::StringRecord) -> Result<AgentMetadata> {
        ...
        if let Some(repairs) = record.get(73) {
            if !repairs.is_empty() {
                agent.repair_recommendations = repairs.split('.').map(|s| s.trim().to_string()).collect();
            }
        }
    ...
    fn rebuild_indexes(&self) -> Result<()> {
        ...
        *stats_write = RegistryStats::new();
        ...
        for agent in agents.values() {
            ...
            *stats_write.agents_by_layer.entry(agent.layer_name().to_string()).or_insert(0) += 1;
        }
