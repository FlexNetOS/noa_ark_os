  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-surface/70 p-5 text-white/70">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold uppercase tracking-[0.3em] text-white/50">Live crew</h3>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {active.length === 0 && <p className="text-sm text-white/40">You are opening the space. Ping the crew!</p>}
        {active.map((item) => (
          <div
            key={item.userId}
          ...
          >
            ...
            <div className="leading-tight">
              <div className="text-sm font-medium text-white">{item.member?.name ?? item.userName}</div>
              <div className="text-[11px] uppercase tracking-[0.2em] text-white/40">{item.status}</div>
            </div>
          </div>
        ))}
      </div>
