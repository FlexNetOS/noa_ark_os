...
export function IntegrationStatus({ integrations }: IntegrationStatusProps) {
  return (
    <div className="rounded-3xl border border-white/10 bg-surface/70 p-5 text-white/70">
      <h3 className="text-sm font-semibold uppercase tracking-[0.3em] text-white/50">Platform signals</h3>
      <div className="mt-4 space-y-3">
        {integrations.map((integration) => (
          <div key={integration.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="flex items-center justify-between">
              ...
              <span className={`rounded-full border px-3 py-1 text-[11px] uppercase tracking-[0.2em] ${statusStyles[integration.status] ?? statusStyles.healthy}`}>
                {integration.status}
              </span>
            </div>
          </div>
        ...
        {integrations.length === 0 && (
          <p className="text-sm text-white/40">No integrations yet — connect CRC, CI/CD, or runtime modules to light this up.</p>
        )}
      </div>
    </div>
  );
