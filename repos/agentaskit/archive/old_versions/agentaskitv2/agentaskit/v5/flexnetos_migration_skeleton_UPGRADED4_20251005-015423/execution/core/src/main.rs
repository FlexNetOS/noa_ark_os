...
fn verify_minisign(manifest: &Path) -> Result<()> {
    if let Ok(pubkey) = std::env::var("FLEX_MINISIGN_PUB") {
        ...
        println!("[verify] minisign not enforced (FLEX_MINISIGN_PUB unset).");
    }
    Ok(())
}

fn sealed_ok(manifest: &Path) -> bool {
    let imm = Command::new("lsattr").arg(manifest).output()
        .map(|o| String::from_utf8_lossy(&o.stdout).contains('i')).unwrap_or(False);
    let ver = Command::new("fsverity").arg("measure").arg(manifest).output()
...
fn verify_manifest(root: &Path) -> Result<()> {
    ...
    if std::env::var("FLEX_ENFORCE_SEAL").ok().as_deref() == Some("1") {
        if !sealed_ok(&manifest) {
            return Err(anyhow!("[verify] seal enforcement is on but manifest is neither immutable nor fs-verity protected."));
        } else {
            println!("[verify] seal check passed (immutable or fs-verity).");
        }
    }
