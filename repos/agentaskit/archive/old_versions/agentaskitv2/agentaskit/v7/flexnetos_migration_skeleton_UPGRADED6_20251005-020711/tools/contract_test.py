...
def main():
    ...
    if not ok:
        print("[contract-test] missing IDL or golden samples", file=sys.stderr); sys.exit(2)
    capnp = shutil.which("capnp"); strict = os.environ.get("CAPNP_STRICT","0") == "1"
    if capnp:
        try:
            ...
            if strict: print(msg, file=sys.stderr); sys.exit(2)
            else: print(msg + " (warning only)")
