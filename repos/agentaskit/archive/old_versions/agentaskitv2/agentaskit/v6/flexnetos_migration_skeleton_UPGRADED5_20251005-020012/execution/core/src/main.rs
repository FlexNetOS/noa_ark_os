...
fn verify_minisign(manifest: &Path) -> Result<()> {
    if let Ok(pubkey) = std::env::var("FLEX_MINISIGN_PUB") {
        let sig = if manifest.extension().and_then(|s| s.to_str()) == Some("sha256") {
            manifest.with_extension("sha256.minisig")
        } else {
            manifest.with_extension("minisig")
        };
        if sig.exists() {
            let out = Command::new("minisign")
                ...
                .output();
            if let Ok(o) = out {
                if o.status.success() { println!("[verify] minisign OK"); return Ok(()); }
                else { return Err(anyhow!("[verify] minisign failed: {}", String::from_utf8_lossy(&o.stderr))); }
            }
        } else {
            return Err(anyhow!("[verify] minisign pubkey set but signature not found: {}", sig.display()));
        }
    } else { println!("[verify] minisign not enforced (FLEX_MINISIGN_PUB unset)."); }
    Ok(())
}

// Detect if the filesystem containing `p` is mounted read-only via /proc/self/mountinfo
fn is_mount_ro(p: &Path) -> bool {
    if let Ok(mi) = std::fs::read_to_string("/proc/self/mountinfo") {
        // Find the mountpoint that is a prefix of p
        let mut best_len = 0usize;
        let mut ro = false;
        for line in mi.lines() {
            // mountinfo fields: see man; mountpoint is 5th field after " - " separator.
            // We'll split and get (mount point, options)
            let parts: Vec<&str> = line.split(' ').collect();
            if parts.len() < 10 { continue; }
            let mount_point = parts[4];
            // find " - " separator
            let mut idx = 0usize;
            for (i,w) in parts.iter().enumerate() { if *w == "-" { idx = i; break; } }
            if idx+3 >= parts.len() { continue; }
            let sopts = parts[5]; // super options before "-"
            if p.starts_with(mount_point) {
                if mount_point.len() > best_len {
                    best_len = mount_point.len();
                    ro = sopts.contains("ro," ) || sopts.ends_with(",ro") || sopts=="ro";
                }
            }
        }
        return ro;
...
fn verify_mount_policy(root: &Path) -> Result<()> {
    if std::env::var("FLEX_ENFORCE_MOUNT_RO").ok().as_deref() == Some("1") {
        let man = root.join("artifacts").join("MANIFEST.sha256");
        if !is_mount_ro(&man) {
            return Err(anyhow!("[verify] mount policy: artifacts/ must be on a read-only mount"));
        } else { println!("[verify] mount policy OK (ro)"); }
    }
    Ok(())
}
