...
user input → 4D (Deconstruct → Diagnose → Develop → Deliver) → subject.todo (Inbox) → (Under Analysis) → (Ready for Promotion) → Create TASK STUB (agentask.todo) → Author → Index/Sign → Seal/Policy → Tri-Run → Merge(D) → Verify/Contract → WASM → Run Core → Anchor → Promote
...
- Execute: follow 9-phase steps with evidence and hashes (PRE/POST)
...
Note: Transitions are enforced by the “Automation Gates and Transitions” section.
...
- Anchors: anchors/anchor-${TASK_ID}.json
...
3. Create TASK STUB (agentask.todo Current Tasks)
...
- Action: write HASHES_PRE.txt; set manifest.phases.index_sign.hashes_pre_file
...
- Gate to Run Core: wasm.built=true
...
- Gate to Anchor: run_core.done=true

12. Anchor

- Inputs: SOT.md path
- Action: append sot_entry for TASK_ID; write anchor receipt
- Evidence: SOT entry present with evidence links; anchors/anchor-${TASK_ID}.json
...
per_phase_contract:
  ...
  anchor:
    require: [manifest.phases.anchor.receipt, manifest.phases.anchor.sot_path]
  promote:
