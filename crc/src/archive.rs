...
use crate::{SourceType, ArchiveInfo, ArchiveIndex, FileEntry, Error, Result};
...
impl ArchiveManager {
    ...
    pub async fn archive_drop(
        ...
        let archive_filename = format!("{}_{}_{}.{}", drop_id, source_type_short(&source_type), timestamp, extension);
        ...
        info!("  Compressed: {} bytes ({:.2} MB)", archive_size, archive_size as f64 / 1_048_576.0);
        ...
        // Store archive info
        self.archives.insert(drop_id.to_string(), archive_info.clone());
    ...
    pub async fn cleanup_old_archives(&self) -> Result<CleanupReport> {
        ...
        for (source_type, retention_days) in &self.config.retention_days {
            ...
            info!("  Checking {:?} archives (retention: {} days)", source_type, retention_days);
            ...
            while let Some(entry) = entries.next_entry().await? {
                ...
                if let Ok(created) = metadata.created() {
                    ...
                    if age_days > *retention_days as u64 {
                        info!("    Removing old archive: {} (age: {} days)", path.display(), age_days);
        ...
        info!("✓ Cleanup complete: {} archives removed, {:.2} MB freed",
