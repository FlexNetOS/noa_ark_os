...
- **Policy Enforcement** – `PolicyEnforcer` delegates to `core::security` so user permissions remain the source of truth.
...
If policy enforcement or registry data appears corrupted, re-run `cargo test` to reproduce locally and escalate to the Core
Security maintainers listed in `core/README.md`.
